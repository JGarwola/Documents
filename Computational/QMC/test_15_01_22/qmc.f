...
c space dimensions
       ...
       read(word(2),*)ndim
c constant potential for sr-lr splitter potential
       elseif(word(1).eq.'v0')then
        read(word(2),*)v0
c tail and shift for sr-only potential
       elseif(word(1).eq.'vshift')then
        read(word(2),*)vshift
       elseif(word(1).eq.'vtail')then
        read(word(2),*)vtail
...
c sofk
       elseif(word(1).eq.'sofk')then
        nsofk=nsofk+1
c       do i=1,mtypes
c        if(word(2).eq.typename(i))it=i
c       enddo
c       do i=1,mtypes
c        if(word(3).eq.typename(i))jt=i
c       enddo
c       isofk(it,jt)=nsofk
c       isofk(jt,it)=nsofk
c       pp_dist(it,jt)=1
...
c sofk
      ...
      do i=1,nsofk
       ...
       j_prop_start(iname)=n_props+1
       j_prop_count(iname)=10 ! nk
       n_props=n_props+10 ! nk
      enddo
c # props in stack
...
c put a configuration in the stack
      ...
      do i=1,n
       if(putnext.eq.getnext.and.nstack.ne.0)then
        stop'do not store more than 2000 configurations'
c       stop'okkio: stack full'
       endif
       do it=1,ntypes
        if(hbs2m(it).ne.0.d0)then
         do ip=ipfrst(it),iplst(it)
       ...
       if(mytid.eq.0)then
        ...
        do k=1,nproc
         do it=1,ntypes
          iunit=30+it-1
          do ip=ipfrst(it),iplst(it)
           write(iunit,*)(box(el(idim),aux(j+idim)),idim=1,ndim)
           j=j+ndim
          enddo
         enddo
        enddo
...
c age is # of consecutive rejections for a given walker
      ...
      implicit none
      include'qmc.h'
      integer it,jt,ik,ik1,ik2,i,j,k,idim,ip,ifdist
      real*8 c_sofk
...
c        do j=1,nk
c         ik=jsofk-1+(i-1)*nk+j
c         p_new(ik)=sofk(j,i)*c_sofk
c        enddo
         ik=jsofk
         do j=1,10
c         do k=-10,10
           p_new(ik)=sofk(j,1)*c_sofk
c     print*,' i j sofk ',j,k,sofk(j,k)
           ik=ik+1
c         enddo
         enddo
c     print*,'c_sofk ',c_sofk
c     if(.true.)stop
        endif
       enddo
      enddo
...
c potential per particle
      ...
      enddo
c v0 for lr-sr splitter potential
      p_new(jpot(0))=(p_new(jpot(0))+v0)/npnorm
c tail and shift for sr-only potential
      p_new(jpot(0))=p_new(jpot(0))
     &              +vshift*0.5*(npnorm-1)
     &              +vtail*npnorm/volume
      return
      end
...
c        i=irhok(it)
c        call r_set(nk,sofk(1,isofk(it,it)),0.d0)
c        do ik=1,nk
c         ik2=2*ik
c         ik1=ik2-1
c         sofk(ik,isofk(it,it))=rhok(ik1,i)**2
c    &                         +rhok(ik2,i)**2
c        enddo
         tpiel(1)=2*acos(-1.d0)/el(1)
         tpiel(2)=2*acos(-1.d0)/el(2)
         do i=1,10
c         do j=-10,10
           rhokappa=dcmplx(0.d0,0.d0)
           do ip=1,npnorm
            kr=i*tpiel(2)*x_new(2,ip) ! +j*tpiel(2)*x_new(2,ip)
            rhokappa=rhokappa+exp(dcmplx(0.d0,kr))
           enddo
c          if(i.eq.0.and.j.eq.0)rhokappa=dcmplx(0.d0,0.d0)
           sofk(i,1)=rhokappa*dconjg(rhokappa)
c          print*,'i j rhokappa ',i,j,rhokappa
c         enddo
         enddo
        endif
        do jt=it+1,ntypes
...
c     print*,'average 12 ',blk_av(12)
      elseif(what.eq.3)then
c     print*,'average 22 ',blk_av(22),cml_av(22)
...
c sofk
        ...
        do j=1,nsofk
         string=' '
         call write_sofk(m_props,i,iblk,cml_av
     &       ,cml2,cml_norm,blk_av,blk_norm,sofk_filename(j)
     &       ,iunit,0,0,string,el,who)
c        call write_nonscalar_props(m_props,i,nk,iblk,cml_av
c    &       ,cml2,cml_norm,blk_av,blk_norm,sofk_filename(j),iunit,0,0
c    &       ,string)
        enddo
c derivate
        i=jder
...
c write on file filename n non-scalar props starting from the i-th
     ...
     &       ,string,j
      ...
      subroutine write_sofk(m,i,iblk,cml_av,cml2,cml_norm
     &                     ,blk_av,blk_norm,filename,iunit
     &                     ,ifc,istdout,string,el,who)
c write on file filename n non-scalar props starting from the i-th
      implicit none
      integer j,m,i,k,iblk,iunit,istdout,ifc,i0
      real*8 cml_norm,blk_norm,blk_av(m),cml_av(m),cml2(m),err,el(*)
      real*8 q(3)
      character*48 filename
      character*7 string
      character*3 who
      ...
      open(iunit,file=filename(1:j)//'.'//who,status='unknown')
      i0=i
      q(1)=2*acos(-1.d0)/el(1)
c     q(2)=2*acos(-1.d0)/el(2)
      do j=1,10
c     do k=-10,10
       if(iblk.gt.1)then
        err=sqrt(abs((cml2(i)/cml_norm-(cml_av(i)/cml_norm)**2)
...
c      write(iunit,'(4f20.6)')j*q(1),k*q(2),cml_av(i)/cml_norm,err
       write(iunit,'(4f20.6)')j*q(1),cml_av(i)/cml_norm,err
       i=i+1
c     enddo
c     write(iunit,*)' '
      enddo
...
c silvera-goldman potential
     ...
     &    ,rm_x_1_28/8.2482753559d0/! (4.3648d0 in \AA)
      ...
      integer i,i0,nt,stdin_flag,spline_flag,m_parm,jp
      parameter(m_parm=20)
      real*8 r,r0,drt,f(0:nt),df(0:nt),p(m_parm),x,a,b,alpha,beta,aux
      common /c_wrkparm/p,jp
      spline_flag=1
      if(stdin_flag.ne.0)then
...
c menu
     ...
     &                    ,stdin_flag,spline_flag)
      elseif(routinename(i).eq.'mcmillan')then
       call mcmillan(0.d0,drt,0,ngrid(i),ut(0,1,i),ut(0,2,i)
     &                    ,stdin_flag,spline_flag)
