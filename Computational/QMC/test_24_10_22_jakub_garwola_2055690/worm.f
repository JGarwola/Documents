...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine add_v_ss(ndim,xp,x,el,eli,drt,drti,mgrid,ngrid,t,v,ind)
      implicit none
      integer ndim,mgrid,ngrid,i,ind
      real*8 xp(ndim),x(ndim),el(ndim),eli(ndim),drt,drti,t(0:mgrid,4),v
      real*8 r,rvec,rem
      r=0.d0
      do i=1,ndim
       rvec=xp(i)-x(i)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine add_v(ndim,xp,x,el,eli,drt,drti,mgrid,ngrid,t,v,ind)
      implicit none
      integer ndim,mgrid,ngrid,i,ind
      real*8 xp(ndim),x(ndim),el(ndim),eli(ndim),drt,drti,t(0:mgrid,4),v
      real*8 r,rvec,rem
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 rnd,xnew(mdim,0:m),v,p,delta_azione,quantov(0:m)
      if(zsector)return
      ...
      delta_azione=0
      j=0
      call potential(xnew(1,j),v,it,itau,ip_ira,quantov(j),.false.)
      delta_azione=delta_azione-tau*v
      itau=next_tau(itau)
      do j=1,emme
       call potential(xnew(1,j),v,it,itau,0,quantov(j),.false.)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 rnd,xnew(mdim,0:m),v,delta_azione,p
      if(gsector)return
     ...
     &           ,el,eli,ndim,mdim,xnew)
      ...
      do j=1,emme-1
       ip=next(ip,itau)
       itau=next_tau(itau)
       call potential(x(1,ip,itau),v,it,itau,ip,1.d0,.false.)
       delta_azione=delta_azione+tau*v
       call potential(xnew(1,j),v,it,itau,ip,1.d0,.false.)
       delta_azione=delta_azione-tau*v
      enddo
      p=exp(delta_azione)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 v,delta_azione,p,rnd,twosigma2,r2,rho
     ...
     &           ,emme,sigma(it_ira),el,eli,ndim,mdim,xnew)
      delta_azione=0
      j=0
      call potential(xnew(1,j),v,it_ira,itau_ira,ip_ira,0.5d0,.false.)
      delta_azione=delta_azione-tau*v
      itau=next_tau(itau_ira)
      do j=1,emme-1
       call potential(xnew(1,j),v,it_ira,itau,0,1.0d0,.false.)
       delta_azione=delta_azione-tau*v
       itau=next_tau(itau)
      enddo
      j=emme
      call potential(xnew(1,j),v,it_ira,itau,ip_masha,0.5d0,.false.)
      delta_azione=delta_azione-tau*v
...
c     parameter(epsr=1.d20,sigma=1.d0)
      parameter(epsr=5.d1,sigma=1.d0)
      call r_set(4*(mt+1),f(0,1),0.d0)
      do i=i0,nt
       r=r0+drt*i
       if(r.lt.sigma)then
...
c initialize
      ...
      do k=1,1000
       ...
       elseif(word(1).eq.'cycles')then
        read(word(2),*)ncycles
       elseif(word(1).eq.'nofr')then
        read(word(2),*)nofr_stride
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 rnd,quantov(0:m),x0(mdim),xnew(mdim,0:m),delta_azione,v
      real*8 p
      if(gsector)return
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      include 'worm.h'
      integer i,k,l,iunit(mty),itau,iesimo,ip,mk
      parameter(mk=14)
      ...
      elseif(i.eq.3)then
       ...
       do l=-nsofk,nsofk
        do k=-nsofk,nsofk
         write(99,*)l*tpieli(1),k*tpieli(2),sk(l,k)/kount/np(1)
        enddo
        write(99,*)
       enddo
       close(99)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      include 'worm.h'
      integer i,je,jk,jv,jg,jkt(mty),jvt(mty),jgt(mty*(mty+1)/2)
     &       ,k,l,ip,it,jt,itau,iesimo,npty(mty),nppt,kount
     &       ,iunit,g_iunit,gt_iunit(mty*(mty+1)/2),ngridg
      real*8 v,xprev(mdim),d2(mty),dx,etot,ekin,epot,ekinty(mty),drtg
      character*13 string,ztring
      save je,jk,jv,jkt,jvt,jg,jgt,g_iunit,gt_iunit,ngridg,drtg
      if(i.eq.4)then; return
      elseif(i.eq.2)then
       ...
       do itau=1,ntau,10
        ...
        do iesimo=1,np(1)
         ip=jp(iesimo,itau)
         it=itype(ip,itau)
         call potential(x(1,ip,itau),v,it,itau,ip,1.d0,.true.)
         epot=epot+v
        enddo
       enddo
       epot=epot/kount
       ...
       call ngofr(ndim,np(1),1,1,1,vol,drtg,ngridg,gofr)
       do l=0,ngridg
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine ngofr(ndim,np,ntypes,it,jt,vol,drt,ngrid,gofr)
      implicit none
      integer ndim,ntypes,np(ntypes),it,jt,ngrid,i,icall
      real*8 vol,drt,gofr(0:ngrid),sd,f,den
      save sd,icall
      data icall/0/
      if(icall.eq.0)then
       icall=1
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &                                          ,err(k),blk_norm(k)
      ...
      include 'worm.h'
      integer j
      write(6,'(2e19.11,e9.2,e19.11,x,a13)')blk(j),cml(j)
     &                                     ,err(j),blk_norm(j),pname(j)
      return
      end
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,nmove
       ...
       if(s.gt.rnd)then
        ...
        if(i.eq.iswty    )call switch_type
       endif
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &             ,el,eli,twosigma2,r2,rho)
      ...
      delta_azione=0
      do j=0,emme
       call potential(x(1,ip,itau),v,it,itau,ip,quantov(j),.false.)
       delta_azione=delta_azione+tau*v
       ip=next(ip,itau)
       itau=next_tau(itau)
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine potential(xp,v,it,itau,salvo_ip,quanto,iall)
      implicit none
      include 'worm.h'
      integer it,itau,iesimo,kp,kt,itable,salvo_ip,ind,indg
      real*8 xp(mdim),v,vij,quanto
      logical iall
      v=0.d0
     ...
     &            ,mgrid,ngrid,t(0,1,itable),vij,ind) ! cc ,a2,rcut2)
       ...
       do iesimo=1,np(itau)
        ...
        if(kp.ne.salvo_ip)then
         ...
         itable=iv2(it,kt)
         call add_v(ndim,xp,x(1,kp,itau),el,eli,drt,drti
     &            ,mgrid,ngrid,t(0,1,itable),v,ind) ! cc ,a2,rcut2)
        endif
       enddo
       v=quanto*v
      endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 quantov(0:m),delta_azione,v,p,rnd
      if(zsector)return
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      logical ceira
      integer emme,ip,itau,it,j
      real*8 quantov(0:m),v,delta_azione,p,rnd
      if(zsector)return
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      elseif(irestart.lt.0)then ! legge np posizioni
       open(10,file='worm.init',status='unknown')
       write(*,*)'coordinate iniziali'
       do ip=1,mnp
        read(10,*,end=1)(x0(l),l=1,ndim),it0
        write(*,*)(x0(l),l=1,ndim),it0
        do itau=1,ntau
         do l=1,ndim
          x(l,ip,itau)=x0(l)
         enddo
...
c     if(nppt(it,itau_zeta).lt.1)return
     ...
     &           ,emme,sigma(it),el,eli,ndim,mdim,xnew)
      delta_azione=0
      j=0
      call potential(x(1,ip_ira,itau_ira),v,it,itau_ira,ip_ira,0.5d0
     &              ,.false.)
      delta_azione=delta_azione-tau*v
      call potential(x(1,ip_zeta,itau_zeta),v,it,itau_zeta,ip_zeta,.5d0
     &              ,.false.)
      delta_azione=delta_azione+tau*v
      ip=next(ip_zeta,itau_zeta)
      itau=next_tau(itau_zeta)
      do j=1,emme-1
       call potential(x(1,ip,itau),v,it,itau,ip,1.0d0,.false.)
       call masha_correzione(x(1,ip,itau),v,it,itau)
       delta_azione=delta_azione+tau*v
       call potential(xnew(1,j),v,it,itau,ip,1.0d0,.false.)
       call masha_correzione(xnew(1,j),v,it,itau)
       delta_azione=delta_azione-tau*v
       ip=next(ip,itau)
       itau=next_tau(itau)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine switch_type
      implicit none
      include 'worm.h'
      integer ip,itau,it,jt,ip0,k,j,irandom_number,ip1,itau1
      if(gsector)return
      itau=1
      if(np(itau).eq.0)return
      j=irandom_number(np(itau))
      ip=jp(j,itau)
      it=itype(ip,itau)
      jt=irandom_number(ntypes)
      if(it.eq.jt)return
      att(iswty)=att(iswty)+1
      acc(iswty)=acc(iswty)+1
      ip0=ip
      do k=1,np(itau)
       do j=1,ntau
        itype(ip,itau)=jt
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 rnd,dr(mdim),xnew(mdim),delta_azione,v,p,delta
      if(gsector)return
      itau0=1
      if(np(itau0).eq.0)return
      delta=el(1)/emme_bar(idisplace)
      j=irandom_number(np(itau0))
      ip0=jp(j,itau0)
      it=itype(ip0,itau0)
      ip=ip0
      ...
      do j=1,ntau
       ...
       enddo
       call potential(xnew        ,v,it,itau,ip,1.0d0,.false.)
       delta_azione=delta_azione-tau*v
       ip=next(ip,itau)
       itau=next_tau(itau)
      enddo
