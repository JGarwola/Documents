...
#       $Revision: 1.111 $      $Date: 2020/05/22 07:15:47 $
#
############################################################################
# DESCRIPTION:
#
...
case *Linux*:
  ...
  if (`uname --machine` == "alpha") then
    ...
    # Cavium ThunderX2, AppliedMicro X-Gene, NVIDIA Jetson TX1
    set ARCH=LINUXARM64
...
if ("$VMDRUNDEBUGGER" == "1") then
...
else if ($ARCH == "CRAY_XC") then
  ##
  ## CSCS Piz Daint Cray XC50 and similar machines
  ##
  ## On Piz Daint, GLX- or EGL-enabled VMD builds require a special module
  ## to be loaded before VMD is launched:
  # setenv DISPLAY :0
  # module add eglibs

  ## 
  ## On recent XC machines, SLURM is the default scheduler
  ## and MPI-enabled VMD jobs are run with syntax akin to this:
  ##   srun -C gpu -n 128 --ntasks-per-node=1 \
  ##     /usr/local/bin/vmd193 -dispdev text -e script.tcl
  ##
  ## If we don't find SLURM job launch environment variables set,
  ## then we can reasonably guess that we are running outside of
  ## the queuing system, and MPI should be disabled:
  if ($?SLURM_NTASKS == "0") then
  ...
  ## Otherwise, we assume we're running with MPI enabled...
  $vmdprefixcmd "$MASTERVMDDIR/$execname" $*

else if ($ARCH == "BLUEWATERS" || $ARCH == "CRAY_XK") then
  ##
